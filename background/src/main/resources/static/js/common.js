...
layui.use(['form', 'jquery', 'laydate', 'layer', 'laypage', 'dialog',   'element'], function() {
	...
	$('#table-list').on('click', '.del-user-btn', function() {
			var url=$(this).attr('data-url');
			var id = $(this).attr('data-id');
			dialog.confirm({
				message:'您确定要进行删除吗？',
				success:function(){
					$.post("user-delete.do"+"?id="+id);
					layer.msg('删除成功')
				},
				cancel:function(){
					layer.msg('操作取消')
	...
	$('#table-list').on('click', '.del-manager-btn', function() {
			var url=$(this).attr('data-url');
			var id = $(this).attr('data-id');
			dialog.confirm({
				message:'您确定要进行删除吗？',
				success:function(){
					$.post("manager-delete.do"+"?id="+id);
					layer.msg('删除成功')
				},
				cancel:function(){
					layer.msg('操作取消')
	...
	$('#table-list,.tool-btn').on('click', '.go-btn', function() {
		...
		if(status === '1'){
			dialog.confirm({
				message:'您确定要封禁该用户吗？',
				success:function(){
					$.post("user-status-change.do"+"?id="+id+"&status="+1);
					layer.msg('封禁成功')
				},
				cancel:function(){
					layer.msg('操作取消')
			...
			dialog.confirm({
				message:'您确定要解封该用户吗？',
				success:function(){
					$.post("user-status-change.do"+"?id="+id+"&status="+status);
					layer.msg('解封成功')
				},
				cancel:function(){
					layer.msg('操作取消')
	...
	//商品状态
	$('#table-list,.tool-btn').on('click', '.go-product-btn', function() {
		var url=$(this).attr('data-url');
		var id=$(this).attr('data-id');
		url=url+"?id="+id;
		//将iframeObj传递给父级窗口,执行操作完成刷新
		parent.page("更新", url, iframeObj, w = "700px", h = "620px");
		return false;

	}).mouseenter(function() {
		dialog.tips('更新', '.go-product-btn');
