...
public interface ProductRepository  extends JpaRepository<Product,Long>,JpaSpecificationExecutor<Product>,PagingAndSortingRepository<Product,Long>{
    ...
    //根据单个商品id返回商品的需要信息
    @Query(value = "select id,name,picture,picture_url,category_first,category_second,cprice,stock,detail,status,sales_amount from product x where x.id =?1 ",nativeQuery = true)
