...
![](RackMultipart20210806-4-1lbvg8r_html_866138a81a793a46.png)
...
![](RackMultipart20210806-4-1lbvg8r_html_3fa98bfbb9f012b6.png)

1. **后端技术选型详细介绍**

**1.**** 后端主体框架—— ****Spring Boot**** 框架**

1.1 什么是Spring Boot ?

Spring Boot 是由 Pivotal 团队提供的全新框架，其设计目的是用来简化新 Spring 应用的初始搭建以及开发过程。该框架使用了特定的方式来进行配置，从而使开发人员不再需要定义样板化的配置。

  1. Spring Boot 的优点——自动装配
...
![](RackMultipart20210806-4-1lbvg8r_html_52f0f8417e2ec876.png)
...
Spring Data JPA是Spring提供对数据库访问的技术，其中包括关系型数据库、非关系型数据库（No-sql数据库）、以及map-reduce（大数据Hadoop）框架的访问。

2.2 Spring Data JPA的优点？

（1）简化持久层开发工作

Spring Data JPA可以让我们对于基础功能的实现在持久层不用去书写sql语句，直接继承Repository接口的Dao接口，开启JPA的注解扫描就可以进行持久层开发。对于一些复杂功能还提供了属性表达式、命名查询、Query注解等方式供我们使用。

（2）简化分页业务处理

Spring Data JPA的Repository接口实现了PagingAndSortingRepository接口，可以直接传参调用实现分页功能。
...
网络安全问题至关重要。作为生鲜购物平台开发项目，涉及到用户个人信息、用户与商家金钱交易等诸多用户个人数据，项目开发的安全性能极其关键。我们选择了基于 Spring AOP 和 Servlet 过滤器的Spring Security安全框架，确保项目拥有很好的安全性能。
...
优 ![](RackMultipart20210806-4-1lbvg8r_html_186b29c4e53e67c7.png)
 点：既可以免去我们手动配置各种拦截器和过滤器的操作，又可以为项目提供更好的安全性能。

![](RackMultipart20210806-4-1lbvg8r_html_799ccbbd8250252c.png) ![](RackMultipart20210806-4-1lbvg8r_html_126cd21590c7354f.png)

3.4 项目中的应用

对于河马先生购物商城web应用，基本的用户注册页、用户登录页等页面可以任何用户无权限访问，而对于用户主页、订单等用户个人信息需要用户权限，并且需要设置拦截，我们通过Spring Security 进行了很好的权限管理。
...
![](RackMultipart20210806-4-1lbvg8r_html_67f9e5fb5285d668.png)
...
但spring security本身是通过session验证，不原生支持token验证，我小组成员为了使项目实现高安全性能与无状态验证，在spring security框架的基础上，手动添加了jwt token过滤层与相应的处理组件，以实现token的发放与验证。

项 ![](RackMultipart20210806-4-1lbvg8r_html_b4d63b091b5ecb03.png)
 ![](RackMultipart20210806-4-1lbvg8r_html_7e41c6b70ecdc7fe.png)
 目实现的流程图如下：

![](RackMultipart20210806-4-1lbvg8r_html_d125418d466e9a1d.png)

**5.**  **分布式搜索和分析引擎——**** Elasticsearch**

5.1 什么是Elasticsearch？

E ![](RackMultipart20210806-4-1lbvg8r_html_8aec7213e75f3958.jpg)
 lasticsearch 是一个分布式、高扩展、高实时的搜索与数据分析引擎。它能很方便的使大量数据具有搜索、分析和探索的能力。

5.2 本项目使用Elasticsearch的原因

（1）相比于传统的数据库，由于采用倒排索引，Elasticsearch的搜索响应时间平均较短。

（2）相对于数据库的精准匹配，Elasticsearch的相关性匹配更贴近人的思维方式。在本次项目中，我们小组采用相关度打分匹配的方式，在返回结果中就会根据分数由高到低排列，分数越高意味着和查询语句越相关。此外， 我们还创新型加入了权重搜索模式，将用户搜索词与商品名称、商品详情、商品类别等不同元素赋予不同的权重，进行综合打分，返回用户一个更加精准的结果。
...
（ ![](RackMultipart20210806-4-1lbvg8r_html_de5e6cdb0066429f.png)
3）我们小组还在Elasticsearch中加入了IK分词器和拼音搜索功能，更能满足中文用户的搜索需求。下图分别为错字搜索和拼音搜索：

![](RackMultipart20210806-4-1lbvg8r_html_b4151a363030b22b.png)
...
![](RackMultipart20210806-4-1lbvg8r_html_16756e294f7262b3.png)
...
![](RackMultipart20210806-4-1lbvg8r_html_996655b7f9f59798.png)

![](RackMultipart20210806-4-1lbvg8r_html_cbb43b8d91a5650a.png)
...
![](RackMultipart20210806-4-1lbvg8r_html_1a3ffb08b5de1c9e.png)
...
![](RackMultipart20210806-4-1lbvg8r_html_9a59686f105f4dc9.jpg)

![](RackMultipart20210806-4-1lbvg8r_html_f00f5c628a713da0.png)项目异常处理实例：

**9.**** 统一的日志记录—— ****SLF4J**** 和 ****Logback**** 日志框架**
...
![](RackMultipart20210806-4-1lbvg8r_html_5083d8e2a62cbcba.gif)

**10.**** 严格遵守 ****API**** 规范 前后端分离高效进行**

10.1RESTful API规范

RESTful 架构可以充分的利用 HTTP 协议的各种功能，是 HTTP 协议的最佳实践。RESTful API 是一种软件架构风格、设计风格，可以让软件更加清晰，更简洁，更有层次，可维护性更好。
...
我们在此次项目开发中完全按照spring Boot + Vue 的前后端分离开发模式。在这一过程中，规范的API接口文档起着举足轻重的作用，保证了前后端开发小组工作的高效有序进行。累计接口文档60余页，近一万字。

![](RackMultipart20210806-4-1lbvg8r_html_ed8ae49b8c2ef57.png)

10.3 postman 规范测试

API接口在开发完成后，后端开发小组成员通过postman测试，确保API接口功能正常，并保证接口接收JSON数据格式与返回JSON数据格式与接口文档保持一致，这样为前后端之间工作的高效有序进行提供了很大的便利。

![](RackMultipart20210806-4-1lbvg8r_html_7cad83d9ecd0f2d7.png)
...
通过对MYSQL数据库的实时检测，将降价商品加入降价商品表，并采用定时功能选择用户所喜爱的最大降价幅度的商品发送消息通知，用户在登陆后便可以接收到通知，通过便捷的弹窗直接进入支付界面，缩短用户思考时间，增加 ![](RackMultipart20210806-4-1lbvg8r_html_bfcc95b69443aa9f.png)
 用户购买机率，实现卖家收益最大化。
...
![](RackMultipart20210806-4-1lbvg8r_html_51a65a358d72e020.gif)
（2）通过使用Redis作为消息队列，异步处理订单入库。

**13**** ．安全性能**

**13.1**  **验证码——**** Secure Random ****随机数**
...
spring security中的BCryptPasswordEncoder方法采用SHA-256 +随机盐+密钥对密码进行加密。SHA系列是Hash算法，不是加密算法，使用加密算法意味着可以解密，但是采用Hash处理，其过程是不可逆的。
...
![](RackMultipart20210806-4-1lbvg8r_html_f19196fa978e95f3.png)
...
![](RackMultipart20210806-4-1lbvg8r_html_5bf1160deaa6439d.png)

1. **收件人表**

该表主要用于存储收件人基本信息、创建时间与更新时间。

![](RackMultipart20210806-4-1lbvg8r_html_57762a55d0e47ff8.png)

1. **商品表**

该表主要用于存储商品基本信息、状态、添加时间以及商品评论相关信息。

![](RackMultipart20210806-4-1lbvg8r_html_9158cc8df55333a.png)

1. **购物车表**

该表主要用于存储用户购物车信息，购物车中相关商品信息。

![](RackMultipart20210806-4-1lbvg8r_html_735d2fa2dddeeb01.png)

1. **订单表**

该表主要用于存储用户订单基本信息，订单相关时间信息。

![](RackMultipart20210806-4-1lbvg8r_html_dc3e6d933adb1f26.png)

1. **订单条目表**

该表主要用于存储用户订单中每个商品的相关信息。

![](RackMultipart20210806-4-1lbvg8r_html_d39ec3107360f4cd.png)

1. **收藏夹表**

![](RackMultipart20210806-4-1lbvg8r_html_622cc2c72be267f.png)该表主要用于存储用户收藏夹相关商品信息和创建时间相关信息。

1. **评论表**

该 ![](RackMultipart20210806-4-1lbvg8r_html_b90ce5d5adc32897.png)
 表主要用于存储用户对于商品的评论基本信息。

1. **验证码表**

该表主要用于存储用户验证码与验证码创建时间，便于进行时间失效检测。

![](RackMultipart20210806-4-1lbvg8r_html_6b4b73ffcef87541.png)

**10**** ．论坛表**

该表主要用于存储用户交流论坛中帖子相关信息。

![](RackMultipart20210806-4-1lbvg8r_html_d3b23a5bb2f0d695.png)
