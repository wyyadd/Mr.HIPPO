...
![](media/image4.png)优点：既可以免去我们手动配置各种拦截器和过滤器的操作，又可以为项目提供更好的安全性能。
...
![](media/image8.png)![](media/image9.png)项目实现的流程图如下：
...
![](media/image11.jpeg)Elasticsearch
...
![](media/image12.png)（3）我们小组还在Elasticsearch中加入了IK分词器和拼音搜索功能，更能满足中文用户的搜索需求。下图分别为错字搜索和拼音搜索：
...
通过对MYSQL数据库的实时检测，将降价商品加入降价商品表，并采用定时功能选择用户所喜爱的最大降价幅度的商品发送消息通知，用户在登陆后便可以接收到通知，通过便捷的弹窗直接进入支付界面，缩短用户思考时间，增![](media/image23.png)加用户购买机率，实现卖家收益最大化。
...
![](media/image24.png)（2）通过使用Redis作为消息队列，异步处理订单入库。
...
> ![](media/image32.png)该表主要用于存储用户收藏夹相关商品信息和创建时间相关信息。

8.  **评论表**

> ![](media/image33.png)该表主要用于存储用户对于商品的评论基本信息。
